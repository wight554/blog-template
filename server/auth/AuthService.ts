...
export class AuthService {
  ...
  async validateUser(username: string, pass: string): Promise<any> {
    ...
    const isPasswordMatch = user && (await this.cryptoService.compare(pass, user.password));
  ...
  async createUser(user: CreateUserDto): Promise<any> {
    ...
    const { password: _password, ...createdUser } = await this.userService.create(payload);
  ...
  async updateUser(userId: string, user: UpdateUserDto): Promise<any> {
    ...
    const { password: _password, ...updatedUser } = await this.userService.update(userId, payload);
