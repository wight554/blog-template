...
const handlers = [
  rest.get('*/api/v1/users', async (_req, res, ctx) => {
    return delay(res(ctx.status(StatusCodes.OK), ctx.json(mockUser)), 10);
  }),
  ...
  rest.post('*/api/v1/auth/logout', async (_req, res, ctx) => {
    return delay(res(ctx.status(StatusCodes.OK)), 10);
  }),
  rest.get('*/api/v1/posts', async (_req, res, ctx) => {
    return delay(
      res(ctx.status(StatusCodes.OK), ctx.json([mockPost, { ...mockPost, id: '2' }])),
      10,
    );
  }),
];
